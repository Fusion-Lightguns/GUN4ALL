...
#ifdef USES_ANALOG
    int8_t analogPinX = 27;
    int8_t analogPinY = 26;
#endif // USES_ANALOG
...
LightgunButtons::Desc_t LightgunButtons::ButtonDesc[] = {
    {btnTrigger, LightgunButtons::ReportType_Internal, MOUSE_LEFT, LightgunButtons::ReportType_Mouse, MOUSE_RIGHT, LightgunButtons::ReportType_Internal, PAD_RT, 15, BTN_AG_MASK}, // Barry says: "I'll handle this."
    ...
    {btnPedal, LightgunButtons::ReportType_Mouse, MOUSE_MIDDLE, LightgunButtons::ReportType_Keyboard, playerSelectBtn, LightgunButtons::ReportType_Gamepad, PAD_X, 15, BTN_AG_MASK2},
    {btnPump, LightgunButtons::ReportType_Mouse, MOUSE_RIGHT, LightgunButtons::ReportType_Keyboard, playerStartBtn, LightgunButtons::ReportType_Gamepad, PAD_Y, 15, BTN_AG_MASK2},
...
//TOGGLES
//button combo for burst 
uint32_t BurstToggleBtnMask = BtnMask_Left;

//button combo for autofire
uint32_t AutofireToggleBtnMask = BtnMask_Right;
...
// button combo to switch camera modes
uint32_t CameraModeMask = BtnMask_Pedal | BtnMask_A;
...
// For serial mode:
    ...
    unsigned int serialSolPulsesLength = 100;         // How long to wait between each solenoid event in a pulse, in ms.
...
void loop()
{
    ...
    switch(gunMode) {
        case GunMode_Pause:
            ...
            } else if(buttons.pressedReleased == BurstToggleBtnMask) {
                BurstFireToggle();
            #ifdef USES_RUMBLE
                    ...
                    SolenoidToggle();
            #endif // USES_SOLENOID
            } else if(buttons.pressedReleased == CameraModeMask) {
                CameraMode();
          ...
          switch(serialInput) {
              ...
              case 'F':
                ...
                if(serialInput == '1') {
                    ...
                    Serial.println("Setting Burst Fire Mode");
                } else if(serialInput == '2') {
                    autofireActive = true;
                    burstFireActive = false;
                    Serial.println("Setting Auto Fire Mode");
                } else if(serialInput == '0') {
                    autofireActive = false;
                    burstFireActive = false;
                    Serial.println("Setting Single Fire Mode");
                }
                break;
              #endif // USES_SOLENOID
              default:
      ...
      if(solenoidActive) {
          if(bitRead(serialQueue, 0)) {                             // If the solenoid digital bit is on,
              ...
              if(!serialSolPulsesLast) {                            // Have we started pulsing?
                  analogWrite(solenoidPin, 255);                         // Start pulsing it on!
                  ...
                  if(currentMillis - serialSolPulsesLastUpdate > serialSolPulsesLength) { // Have we passed the set interval length between stages?
                      if(serialSolPulseOn) {                        // If we're currently pulsing on,
                          analogWrite(solenoidPin, 255);                 // Start pulsing it off.
...
void SetMode(GunMode_e newMode)
{
    ...
    case GunMode_Docked:
        ...
        if(!dockedCalibrating) {
            Serial.println("P.I.G.S");
            Serial.println(PIGS_VERSION, 1);
            Serial.println(PIGS_BOARD);
            Serial.println("Player# " + String(PLAYER_NUMBER));
        }
        break;
    }
