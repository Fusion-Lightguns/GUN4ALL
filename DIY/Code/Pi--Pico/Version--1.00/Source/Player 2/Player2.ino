/*!
 * @file Player2.ino
 * @brief PIGS version 1.00. See github for more info.
 ...
 * @copyright Gonezo, https://github.com/Fusion-Lightguns/P.I.G.S--Pico-Gun-System/blob/plus/LICENSE
...
#define DEVICE_NAME "Piggie#1"
#define DEVICE_VID 0x0321
#define DEVICE_PID 0x0420
...
LightgunButtons::Desc_t LightgunButtons::ButtonDesc[] = {
    ...
    {btnGunC, LightgunButtons::ReportType_Mouse, MOUSE_BUTTON5, LightgunButtons::ReportType_Mouse, MOUSE_BUTTON5, LightgunButtons::ReportType_Gamepad, PAD_LB, 15, BTN_AG_MASK2},
    ...
    {btnReload, LightgunButtons::ReportType_Mouse, MOUSE_RIGHT, LightgunButtons::ReportType_Mouse, MOUSE_RIGHT, LightgunButtons::ReportType_Gamepad, PAD_Y, 15, BTN_AG_MASK2},
};
...
// button combo to send an exit keypress
// this button combo exits retropie, it also brings test menu for mame
uint32_t ExitKeyBtnMask = BtnMask_Calibrate | BtnMask_B;
...
uint32_t ExitPauseModeHoldBtnMask = BtnMask_Reload;

// press any button to cancel the calibration (this is not a button combo)
uint32_t CancelCalBtnMask = BtnMask_Start | BtnMask_Select;
...
uint32_t RunModeNormalBtnMask = BtnMask_B | BtnMask_Select;
uint32_t RunModeAverageBtnMask = BtnMask_B | BtnMask_Start;

// button combination to toggle offscreen button mode in software:
uint32_t OffscreenButtonToggleBtnMask = BtnMask_A | BtnMask_Left;

// button combination to auto fire mode in software:
uint32_t AutofireSpeedToggleBtnMask = BtnMask_A | BtnMask_Right;

// button combination to toggle rumble in software:
uint32_t RumbleToggleBtnMask = BtnMask_A | BtnMask_Up;

// button combination to toggle solenoid in software:
uint32_t SolenoidToggleBtnMask = BtnMask_A | BtnMask_Right;
...
static const ProfileDesc_t profileDesc[ProfileCount] = {
    {BtnMask_A, WikiColor::Cerulean_blue, "Up", "TV Fisheye Lens"},
    {BtnMask_B, WikiColor::Cornflower_blue, "Down", "TV Wide-angle Lens"},
    {BtnMask_Start, WikiColor::Green, "Left", "TV"},
    {BtnMask_Select, WikiColor::Green_Lizard, "Right", "Monitor"}
};
...
void loop1()
{
    while(gunMode == GunMode_Run) {
        ...
        if(buttons.pressedReleased == ExitKeyBtnMask) {
            SendExitKey();
        }
        ...
        if(holdToPause) {
            ...
            } else if(buttons.pressedReleased == BtnMask_Calibrate) {
                // MAKE SURE EVERYTHING IS DISENGAGED:
                #ifdef USES_SOLENOID
                    digitalWrite(solenoidPin, LOW);
                    solenoidFirstShot = false;
                ...
                if(!dockedSaving) {
                    ...
                    // Pins
                        ...
                        switch(sCase) {
                          ...
                          case 13:
                            serialInput = Serial.read(); // nomf
                            btnReload = Serial.parseInt();
                            btnReload = constrain(btnReload, -1, 40);
...
{
    if(isIncrement) {
        if(pauseModeSelection == PauseMode_ExitSignal) {
            pauseModeSelection = PauseMode_Calibrate;
        } else {
            pauseModeSelection++;
        ...
        if(pauseModeSelection == PauseMode_Calibrate) {
            pauseModeSelection = PauseMode_ExitSignal;
        } else {
            pauseModeSelection--;
            #ifdef USES_SWITCHES
                #ifdef USES_SOLENOID
    ...
    switch(pauseModeSelection) {
        ...
        */
        case PauseMode_ExitSignal:
          Serial.println("Selecting: Send Exit key signal");
          #ifdef LED_ENABLE
              LedUpdate(150,0,150);
          #endif // LED_ENABLE
          break;
...
void ExtPreferences(bool isLoad)
{
    ...
    if(!isLoad) {
        ...
        if(customPinsInUse) {
            ...
            btnReload = tempMappings[12];
            #ifdef USES_RUMBLE
            rumblePin = tempMappings[13];
            #endif // USES_RUMBLE
            #ifdef USES_SOLENOID
            solenoidPin = tempMappings[14];
            #ifdef USES_TEMP
            tempPin = tempMappings[15];
            #endif // USES_TEMP
            #endif // USES_SOLENOID
            #ifdef USES_SWITCHES
            #ifdef USES_RUMBLE
            rumbleSwitch = tempMappings[16];
            #endif // USES_RUMBLE
            #ifdef USES_SOLENOID
            solenoidSwitch = tempMappings[17];
            #endif // USES_SOLENOID
            autofireSwitch = tempMappings[18];
            #endif // USES_SWITCHES
            #ifdef FOURPIN_LED
            PinR = tempMappings[19];
            PinG = tempMappings[20];
            PinB = tempMappings[21];
            #endif // FOURPIN_LED
            #ifdef CUSTOM_NEOPIXEL
            customLEDpin = tempMappings[22];
            #endif // CUSTOM_NEOPIXEL
            #ifdef USES_ANALOG
            analogPinX = tempMappings[23];
            analogPinY = tempMappings[24];
            #endif // USES_ANALOG
        }
...
void UpdateBindings(bool offscreenEnable)
{
    ...
    LightgunButtons::ButtonDesc[BtnIdx_Reload].pin = btnReload;
