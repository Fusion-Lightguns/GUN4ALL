 ...
 * Gun4ALL is based on Prow's Enhanced Fork from https://github.com/Prow7/ir-light-gun, 
 * Prow7 is based on the 4IR Beta "Big Code Update" SAMCO project from https://github.com/samuelballantyne/IR-Light-Gun .
 *
...
#define DEVICE_NAME "Piggie 3"
#define DEVICE_VID 0x0323
#define DEVICE_PID 0x0422
...
int8_t btnCalibrate = 6;                               // Everything below are for GCon 2-spec only 
int8_t btnStart = 9;
int8_t btnSelect = 8;
int8_t btnGunUp = 18;
...
int8_t btnGunB = -1;                              // <-- GCon 1-spec
int8_t btnCalibrate = -1;                              // Everything below are for GCon 2-spec only 
int8_t btnStart = -1;
int8_t btnSelect = -1;
int8_t btnGunUp = -1;
int8_t btnGunDown = -1;
...
LightgunButtons::Desc_t LightgunButtons::ButtonDesc[] = {
    ...
    {btnGunA, LightgunButtons::ReportType_Mouse, playerABtn, LightgunButtons::ReportType_Mouse, playerABtn, LightgunButtons::ReportType_Gamepad, PAD_A, 15, BTN_AG_MASK2},
    {btnGunB, LightgunButtons::ReportType_Mouse, playerBBtn, LightgunButtons::ReportType_Mouse, playerBBtn, LightgunButtons::ReportType_Gamepad, PAD_B, 15, BTN_AG_MASK2},
...
// button COMBO to send exit keypress(f4)
// this will exit retropie or bring up test menu in mame
uint32_t ExitKeyBtnMask = BtnMask_Calibrate | BtnMask_A;

// button COMBO to send menu keypress(tab)
// this will exit retropie or bring up test menu in mame
uint32_t MenuKeyBtnMask = BtnMask_Calibrate | BtnMask_B;

// press any button to exit hotkey pause mode back to run mode (this is not a button combo)
uint32_t ExitPauseModeBtnMask = BtnMask_Pump;
...
uint32_t CancelCalBtnMask = BtnMask_Calibrate;

// button combo to skip the center calibration step
uint32_t SkipCalCenterBtnMask = BtnMask_Up;
...
uint32_t RunModeNormalBtnMask = BtnMask_B | BtnMask_Start;
uint32_t RunModeAverageBtnMask = BtnMask_B | BtnMask_Select;
...
uint32_t SolenoidToggleBtnMask = BtnMask_A | BtnMask_Down;
...
void loop1()
{
    while(gunMode == GunMode_Run) {
        ...
        if(buttons.pressedReleased == MenuKeyBtnMask) {
            SendMenuKey();
        }
